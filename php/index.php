...
<html lang="pl">
    <head>
        ...
        <title>Strona główna - Polite Guys</title>
    ...
    <body>
        ...
        <?php
            $sql = "select path_ from images where id = 2";
            $result=mysqli_query($db,$sql);
            $row = mysqli_fetch_row($result);
        ?>
        <section class="header-banner" style="background-image: url(<?php echo $row[0]?>)">
            <div class="header-banner__container">
                <?php
                    $sql = "select about from main where id = 2";
                    $result=mysqli_query($db,$sql);
                    $row = mysqli_fetch_row($result);
                ?>
        ...
        <div class="page-content">
            <section class="page-description">
                <?php
                    $sql = "select about from main where id = 3";
                    $result=mysqli_query($db,$sql);
                    $row = mysqli_fetch_row($result);
                ?>
            ...
            <section class="tiles">
                <?php
                    ...
                    while($row = mysqli_fetch_assoc($result)){?>
                        <a href="<?php echo $row['link']?>"class="single-tile">
                        <span class="single-tile__text"><?php echo $row['about']?></span>
                        </a>
                        <?php
                    }   
                ?>
